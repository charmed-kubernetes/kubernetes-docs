...
def test_update_install_manual(tmpdir):
    original_text = "\n".join(
        [
    ...
    install_manual = tmpdir / "pages" / "k8s" / "install-manual.md"
    Path(install_manual).parent.mkdir(parents=True)
    install_manual.write_text(original_text, "utf-8")
    pw = generator.PageWriter("0.13", Path(tmpdir))
    text = pw.update_install_manual()
    assert install_manual.read_text("utf-8") == text
    assert text.endswith("\n"), "file should end with Line Feed"
    assert text != original_text, "file contents should have changed"
    assert "mock-table" in text.splitlines()[1], "_bundle_table is on line 2"
