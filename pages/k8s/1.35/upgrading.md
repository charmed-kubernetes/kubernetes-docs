...
<div class="p-notification--information is-inline">
  <div markdown="1" class="p-notification__content">
    ...
    <p class="p-notification__message"><strong>Kubernetes</strong> will automatically handle patch releases. This means that the cluster will perform an unattended automatic upgrade between patch versions, e.g. 1.35.1 to 1.35.2. Attended upgrades are only required when you wish to upgrade a minor version, e.g. 1.35.x to 1.36.x.</p>
  </div>
</div>
...
<div class="p-notification--warning is-inline">
  <div markdown="1" class="p-notification__content">
    ...
    <p class="p-notification__message"><strong>Juju compatibility</strong>  The latest current version of Juju (as of the time of the Charmed Kubernetes 1.35 release) is <strong>3.6</strong>. This new major release introduces some breaking changes with previous versions. It is recommended that you upgrade to this new version of Juju, but also be aware of the changes. See the <a href="https://documentation.ubuntu.com/juju/3.6/howto/manage-controllers/#upgrade-a-controller"> Juju documentation</a> for more information.</p>
  </div>
</div>
...
```bash
juju refresh containerd --channel=1.35/stable
```
...
```bash
juju refresh etcd --channel=1.35/stable
```
...
juju deploy metallb --channel 1.35/stable --trust --config namespace=metallb-system-2
```
...
```bash
juju refresh easyrsa --channel=1.35/stable
```
...
```bash
juju refresh flannel --channel=1.35/stable
```
...
```bash
juju refresh kubeapi-load-balancer --channel=1.35/stable
```
...
```bash
juju refresh kubernetes-control-plane --channel=1.35/stable
```

Once the charm has been upgraded, it can be configured to select the desired **Kubernetes** channel, which takes the form `Major.Minor/risk-level`. This is then passed as a configuration option to the charm. So, for example, to select the stable 1.35 version of **Kubernetes**, you would enter:

```bash
juju config kubernetes-control-plane channel=1.35/stable
```
...
```bash
juju refresh kubernetes-worker --channel=1.35/stable
```
...
```bash
juju config kubernetes-worker channel=1.35/stable
```
...
<div class="p-notification--information">
  <div class="p-notification__content">
    ...
    <a href="https://github.com/charmed-kubernetes/kubernetes-docs/edit/main/pages/k8s/1.35/upgrading.md" >edit this page</a>
    or
