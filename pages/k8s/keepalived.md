...
1. Configure the keepalived application. You should substitute a suitable IP address and
    ...
    ```
    Note that the VIP here must be able to route to any of the kubeapi-load-balancer
    instances. While this can be made to work on [OpenStack][openstack-vip], it will
    generally not be possible on public clouds. Thus, it is generally better to
    instead replace kubeapi-load-balancer with a cloud-provided load balancer
    with health checks, such as Octavia or ELB.
