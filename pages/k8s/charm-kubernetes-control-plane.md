...
charm_revision: '0'
context:
  description: Kubernetes-control-plane Charm reference
  title: kubernetes-control-plane charm
keywords: kubernetes-control-plane, charm, config
layout:
- base
- ubuntu-com
markdown_includes:
  nav: kubernetes/docs/shared/_side-navigation.md
permalink: charm-kubernetes-control-plane.html
...
| <a id="table-keystone-policy"> </a> keystone-policy | string | [See notes](#keystone-policy-default) | Policy for Keystone authorization. This is used when a Keystone charm is related to kubernetes-control-plane in order to provide authorization for Keystone users on the Kubernetes cluster. |
| <a id="table-keystone-ssl-ca"> </a> keystone-ssl-ca | string |  | Keystone certificate authority encoded in base64 for securing communications to Keystone. For example: `juju config kubernetes-control-plane keystone-ssl-ca=$(base64 /path/to/ca.crt)` |
...
2.  Append the desired config option to the previous output and apply:
    ```bash
    juju config kubernetes-control-plane api-extra-args="enable-admission-plugins=PodSecurityPolicy"
    ```
...
The process is the same for both the `kubernetes-control-plane` and the
`kubeapi-load-balancer`. The configuration option takes a space-separated list
of extra entries:

```bash
juju config kubernetes-control-plane extra_sans="master.mydomain.com lb.mydomain.com"
...
```bash
juju run-action kubernetes-control-plane ACTION [parameters] [--wait]
```
<div class="row">
  <div class="col-2">
    <h5>
